    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult GenerateDatasets() {
            foreach (string country in countries) {
                IEnumerable<Order> ordersInCountry = db.Orders.Where(order=>order.Customer.Country==country).Include(order=>order.OrderDetails).AsEnumerable();
                
                IEnumerable<ProductCobought> coboughtProducts = ordersInCountry.SelectMany(ordersInCountry=> from lineItem1 in ordersInCountry.OrderDetails
                                                                                                             from lineItem2 in ordersInCountry.OrderDetails
                                                                                                             select new ProductCobought {
                                                                                                                 ProductID = (uint)lineItem1.ProductId,
                                                                                                                 CoboughtProductID = (uint)lineItem2.ProductId
                                                                                                             }).Where(p=>p.ProductID !=p.CoboughtProductID).GroupBy(p=>new {p.ProductID, p.CoboughtProductID}).Select(p=>p.FirstOrDefault()).OrderBy(p=>p.ProductID).ThenBy(p=>p.CoboughtProductID);
        ...
        public IActionResult TrainModels() {
            ...
            foreach(string country in countries) {
                ...
                MatrixFactorizationTrainer mtf = mlContext.Recommendation().Trainers.MatrixFactorization(options);

                ITransformer trainedModel = mtf.Fit(dataView);
        ...
        public IActionResult Cart(int? id) {
            ...
            if (System.IO.File.Exists(GetDataPath("germany-model.zip"))) {
                ...
                var predictedEngine  =mlContext.Model.CreatePredictionEngine<ProductCobought, Recommendation>(modelGermany);

                var products = db.Products.ToArray();

                foreach (var item in model.Cart.Items) {
                    var topThree = products.Select(product=>predictedEngine.Predict(new ProductCobought {
                ...
                model.Recommendations = model.Recommendations.OrderByDescending(rec=>rec.Score).Take(3).ToList();
            }
